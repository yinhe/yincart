...
class BrandController extends Controller {
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Brand'])) {
            ...
            $img = CUploadedFile::getInstance($model, 'brand_logo');
            if ($img) {
                if ($img->size > 2000000) {
                    ...
                    $dir = dirname(Yii::app()->basePath) . '/upload/brand/';
                    $img_src = $dir . md5(time()) . '.' . $extensionName;
                    $img1 = md5(time()) . '.' . $extensionName;
                    $model->brand_logo = $img1;
                }
            }
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Brand'])) {
            $model->attributes = $_POST['Brand'];
            unset($_POST['Brand']['brand_logo']);
            $Brand = Brand::model()->findByPk($id);
            $filesize = $_FILES['Brand']['size']['brand_logo'];
            if ($filesize != 0) {
                $img = CUploadedFile::getInstance($model, 'brand_logo');
                $extensionName = explode('.', $img->getName());
                $extensionName = $extensionName[count($extensionName) - 1];
            ...
            if ($model->save()) {
                if ($filesize != 0) {
                    @unlink(dirname(Yii::app()->basePath) . '/upload/brand/' . $Brand->brand_logo);
                    $img->saveAs($img_src);
                }
